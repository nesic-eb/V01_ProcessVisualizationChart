...
function getProcessChartData() {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      $('#chartDesignCode').val(response[0]);
  ...
  $.ajax({
    ...
    success: function (response) {
        ...
        if (response[0].Data[i].ProcessProcedureID == process_procedureID) {
          ...
          for (var j = 0; j < colNum; j++) {
            ...
            if (j == 0) {
              ...
              header += "<th style='text-align: center; width: 230px;'>" +
      ...
      getProcessChartImageData(rowNum, colNum);
  ...
  function getProcessChartImageData(rowNum, colNum) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        console.log("image list length = " + data_array.length)
        ...
        function custom_template(obj) {
          ...
          if (text == "") {
            template = $("<div style='width:100px;height:30px;margin-top:6px;'></div>");
            return template;
          }
          else {
            if (data && data['img_src']) {
              img_src = data['img_src'];
              template = $("<div><img src=\"" + img_src + "\" style=\"width:100px;height:40px;margin-left:36px;margin-top:6px;\"/></div>");
              return template;
            }
